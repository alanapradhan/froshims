...
$updateScoreReported = mysql_query("UPDATE beta_schedule SET score = 1 WHERE GId='$gid' and type='$type'");
	/*
	$result = mysql_query("UPDATE schedule SET Forfeit = '$forfeit' WHERE gid='$gid'");

	$findwinner2 = mysql_query("Select * FROM schedule WHERE
gid='$gid' AND Team1= '$forfeit'");

	if($row = mysql_fetch_array($findwinner2)){
		$winner= addslashes($row["Team2"]);
		$insertloser = mysql_query("UPDATE schedule SET Win = '$winner' WHERE gid='$gid'");
	}
	
	else{
		$findwinner1 = mysql_query("Select * FROM schedule WHERE gid='$gid' AND Team2= '$forfeit'");
		$row = mysql_fetch_array($findwinner1);
		$winner= addslashes($row["Team1"]);
                $insertwinner=  mysql_query("UPDATE schedule SET Win = '$winner' WHERE gid='$gid'"); 
	}
	*/
